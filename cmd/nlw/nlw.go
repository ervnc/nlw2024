...
func run(ctx context.Context) error {
	...
	logger = logger.Named("nlw_app")
	defer logger.Sync()

	pool, err := pgxpool.New(ctx, fmt.Sprintf(
		"user=%s password=%s host=%s port=%s dbname=%s",
		os.Getenv("NLW_DATABASE_USER"),
		os.Getenv("NLW_DATABASE_PASSWORD"),
		os.Getenv("NLW_DATABASE_HOST"),
		os.Getenv("NLW_DATABASE_PORT"),
		os.Getenv("NLW_DATABASE_NAME"),
	))
